/* eslint-disable react/jsx-key */
// icons
import {
  HiHome,
  HiUser,
  HiViewColumns,
  HiRectangleGroup,
  HiChatBubbleBottomCenterText,
  HiEnvelope,
} from "react-icons/hi2";

// nav data
const navData = [
  { name: "Inicio", path: "/", icon: <HiHome /> },
  { name: "Mi Perfil", path: "/about", icon: <HiUser /> },
  { name: "que propongo", path: "/services", icon: <HiRectangleGroup /> },
  { name: "galeria", path: "/work", icon: <HiViewColumns /> },
  {
    name: "en redes",
    path: "/testimonials",
    icon: <HiChatBubbleBottomCenterText />,
  },
  {
    name: "contactame",
    path: "/contact",
    icon: <HiEnvelope />,
  },
];

import Link from "next/link";
import { useRouter } from "next/router";

const Nav = () => {
  const router = useRouter();
  const pathname = router.pathname;
  return (
    <nav className="flex flex-col items-center xl:justify-center gap-y-4 fixed h-max bottom-0 mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen">
      <div className="flex w-full xl:flex-col items-center justify-between xl:justify-center gap-y-10 px-4 md:px-40 xl:px-0 h-[50px] xl:h-max py-8 bg-white/10 backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full">
        {navData.map((li, index) => {
          return (
            <Link
              className={`${
                li.path === pathname && "text-accent"
              } relative flex items-center group hover:text-[#ffcc00] transition-all duration-300`}
              href={li.path}
              key={index}
            >
              <div className="absolute pr-14 right-0 hidden xl:group-hover:flex">
                <div className="bg-white relative flex text-primary items-center p-[6px] rounded-[3px]">
                  <div className="text-[15px] leading-none font-semibold capitalize">
                    {li.name}
                  </div>
                  <div className="border-solid border-l-white border-l-8 border-y-transparent border-y-[6px] border-r-0 absolute -right-2"></div>
                </div>
              </div>
              <div className="xl:text-2xl">{li.icon}</div>
            </Link>
          );
        })}
      </div>
    </nav>
  );
};

export default Nav;
